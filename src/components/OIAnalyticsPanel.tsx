  ...
  return (
    <div className="flex flex-col h-full bg-fw-surface">
      ...
      <div className="flex items-center justify-between px-4 py-2 border-b border-fw-border">
        ...
        <div className="flex items-center gap-2">
          <div className="flex gap-1">
            {symbols.map(s => (
              <button
                key={s}
                onClick={() => setSymbol(s)}
                className={`text-xs px-2 py-0.5 rounded ${symbol === s ? 'bg-fw-accent text-white' : 'text-fw-text-secondary hover:bg-fw-hover'}`}
              >
                {s}
              </button>
            ))}
          ...
          <div className="flex gap-0.5 border border-fw-border rounded">
            <button onClick={() => setViewMode('chart')} className={`text-xs px-2 py-0.5 ${viewMode === 'chart' ? 'bg-fw-accent text-white rounded' : 'text-fw-text-secondary'}`}>Chart</button>
            <button onClick={() => setViewMode('table')} className={`text-xs px-2 py-0.5 ${viewMode === 'table' ? 'bg-fw-accent text-white rounded' : 'text-fw-text-secondary'}`}>Table</button>
          </div>
        </div>
      </div>
      ...
      {summary && (
        <div className="grid grid-cols-4 gap-2 px-3 py-2 border-b border-fw-border">
          <div className="kpi-card !p-2">
            <div className="text-xs text-fw-text-secondary uppercase">PCR</div>
            <div className={`text-sm font-mono font-bold ${summary.pcr > 1 ? 'text-green' : 'text-red'}`}>{summary.pcr.toFixed(2)}</div>
          </div>
          <div className="kpi-card !p-2">
            <div className="text-xs text-fw-text-secondary uppercase">Max Pain</div>
            <div className="text-sm font-mono font-bold text-fw-text">{summary.maxPainStrike}</div>
          </div>
          <div className="kpi-card !p-2">
            <div className="text-xs text-fw-text-secondary uppercase">Highest Call OI</div>
            <div className="text-sm font-mono font-bold text-red">{summary.highestCallOiStrike}</div>
          </div>
          <div className="kpi-card !p-2">
            <div className="text-xs text-fw-text-secondary uppercase">Highest Put OI</div>
      ...
      <div className="flex-1 overflow-y-auto px-3 py-2">
        {loading && <div className="flex items-center justify-center h-20 text-fw-text-secondary text-xs">Loading OI data...</div>}
          ...
          <div className="space-y-1">
            {data.map(d => (
              <div key={d.strike} className="flex items-center gap-1 h-5">
                ...
                <div className="w-14 text-center text-xs font-mono font-bold text-fw-text flex-shrink-0">{d.strike}</div>
        ...
        {!loading && data.length === 0 && (
          <div className="flex items-center justify-center h-20 text-fw-text-secondary text-xs">No OI data available</div>
        )}
      </div>
    </div>
  );
