...
export function DrawingLayersPanel({ items, onClose, onHighlight, onToggleVisibility, onDelete }: Props) {
  return (
    <div
      className="absolute left-[34px] top-4 z-[200] w-[220px] bg-[#14172200] backdrop-blur-sm"
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between px-3 py-2 border-b border-fw-border/50">
          <span className="text-[11px] font-semibold text-fw-text">Layers</span>
          <div className="flex items-center gap-2">
            <span className="text-[9px] text-fw-text-muted">{items.length} object{items.length !== 1 ? 's' : ''}</span>
        ...
        <div className="max-h-[320px] overflow-y-auto">
          {items.length === 0 ? (
            <div className="py-6 text-center text-[10px] text-fw-text-muted">No drawings yet</div>
          ) : (
            items.map((item, idx) => (
              <div
                key={item.id}
              ...
              >
                ...
                {/* Label */}
                <span className="flex-1 text-[10px] text-fw-text truncate">
                  {TYPE_LABEL[item.type] ?? item.type}
                  <span className="text-fw-text-muted ml-1 text-[9px]">#{idx + 1}</span>
                </span>
