    ...
    <div className="h-full overflow-y-auto px-3 py-2">
      ...
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-base font-bold text-fw-text">Risk Management Dashboard</span>
        </div>
        <div className={cn('flex items-center gap-2 px-3 py-1 rounded-md', riskBg)}>
          <span className="text-xs text-fw-text-secondary">Risk Score:</span>
          <span className={cn('text-lg font-bold font-mono', riskColor)}>{riskScore}</span>
          <span className={cn('text-xs font-semibold', riskColor)}>{riskLevel}</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-2">
        <div className="bg-fw-bg border border-fw-border rounded px-3 py-2">
          <div className="text-xs text-fw-text-secondary uppercase mb-1">Current Streak</div>
          <div className={cn('text-md font-bold font-mono', streakData.type === 'win' ? 'text-green' : streakData.type === 'loss' ? 'text-red' : 'text-fw-text-secondary')}>
            {streakData.count > 0 ? `${streakData.count} ${streakData.type === 'win' ? 'Wins' : 'Losses'}` : 'No data'}
          </div>
        </div>
        <div className="bg-fw-bg border border-fw-border rounded px-3 py-2">
          <div className="text-xs text-fw-text-secondary uppercase mb-1">Recent (Last 20)</div>
          <div className="flex items-center gap-2">
            <span className="text-base font-mono text-green">{recentWins}W</span>
            <span className="text-xs text-fw-text-secondary">/</span>
            <span className="text-base font-mono text-red">{recentLosses}L</span>
          </div>
        </div>
        <div className="bg-fw-bg border border-fw-border rounded px-3 py-2">
          <div className="text-xs text-fw-text-secondary uppercase mb-1">Risk Alerts</div>
          <div className="space-y-0.5">
            {dailyLossPct >= 80 && <div className="text-xs text-red font-medium">⚠ Near daily limit</div>}
            {drawdownPct >= 80 && <div className="text-xs text-red font-medium">⚠ Near max DD</div>}
            {marginUtilization >= 80 && <div className="text-xs text-orange-400 font-medium">⚠ High margin use</div>}
            {dailyLossPct < 80 && drawdownPct < 80 && marginUtilization < 80 && (
              <div className="text-xs text-green font-medium">✓ All within limits</div>
            )}
          </div>
        </div>
      </div>
...
function MetricCard({ label, value, color }: { label: string; value: string; color?: 'green' | 'red' | 'orange' }) {
  ...
  return (
    <div className="bg-fw-bg border border-fw-border rounded px-2 py-1.5">
      ...
      <div className={cn('text-base font-bold font-mono truncate', textColor)}>{value}</div>
    </div>
  );
}
