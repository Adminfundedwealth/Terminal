  ...
  return (
    <div className="h-full flex flex-col bg-fw-bg select-none">
        ...
        {Array.from({ length: levels }, (_, i) => {
          ...
          return (
            <div key={i} className="grid grid-cols-[50px_1fr_80px_80px_1fr_50px] items-center relative h-[24px] border-b border-fw-border/5 group hover:bg-fw-hover/10">
              ...
              {/* Bid # Orders */}
              <div className="relative text-center text-[14px] font-mono text-fw-text-muted tabular-nums">
                {bid?.orders || ''}
              </div>

              {/* Bid Qty */}
              <div
                className="relative text-right pr-2 text-[13px] font-mono tabular-nums text-green/80 font-semibold cursor-pointer hover:text-green"
              ...
              {/* Ask Qty */}
              <div
                className="relative text-left pl-2 text-[13px] font-mono tabular-nums text-red/80 font-semibold cursor-pointer hover:text-red"
                onClick={() => ask && setOrderForm({ price: ask.price, side: 'SELL' })}
              >
                {ask ? fmtQty(ask.qty) : ''}
              </div>
      ...
      <div className="border-t border-fw-border bg-fw-surface-2 px-3 py-2 flex-shrink-0">
        ...
        <div className="text-[13px] text-fw-text-muted mt-1 text-center">
