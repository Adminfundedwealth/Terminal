            ...
            <tr key={pos.id} className="group">
              ...
              <td>
                <div className="flex items-center gap-0.5 flex-wrap">
                  ...
                  {/* Break Even */}
                  <button onClick={() => handleBreakEven(pos.id)} className="px-1.5 py-0.5 rounded text-[9px] font-bold bg-fw-bg border border-fw-border text-fw-text-secondary hover:text-fw-accent hover:border-fw-accent transition-colors" title="Break Even — Set SL at entry price">BE</button>

                  {/* Take Profit */}
                  <button onClick={() => setTpInput(tpInput?.id === pos.id ? null : { id: pos.id, price: '' })} className={cn('px-1.5 py-0.5 rounded text-[9px] font-bold bg-fw-bg border border-fw-border text-fw-text-secondary hover:text-green hover:border-green transition-colors', tpInput?.id === pos.id && 'border-green text-green')} title="Take Profit">TP</button>
                  ...
                  {slInput?.id === pos.id && (
                    <div className="flex items-center gap-1 ml-1 pl-1.5 border-l border-red-800/40">
                      <input type="number" placeholder="SL Price" value={slInput.price} onChange={e => setSlInput({ ...slInput, price: e.target.value })} className="w-20 h-5 bg-fw-bg border border-red-800/40 rounded text-[10px] font-mono text-fw-text px-1.5 outline-none focus:border-red" autoFocus />
                      <button onClick={() => handleSL(pos.id, slInput.price)} className="px-1.5 py-0.5 text-[9px] font-bold bg-red-900/30 text-red-400 border border-red-800/40 rounded">Set</button>
                    </div>
                  )}
                  ...
                  {tpInput?.id === pos.id && (
                    <div className="flex items-center gap-1 ml-1 pl-1.5 border-l border-green-800/40">
                      <input type="number" placeholder="TP Price" value={tpInput.price} onChange={e => setTpInput({ ...tpInput, price: e.target.value })} className="w-20 h-5 bg-fw-bg border border-green-800/40 rounded text-[10px] font-mono text-fw-text px-1.5 outline-none focus:border-green" autoFocus />
                      <button onClick={() => handleTP(pos.id, tpInput.price)} className="px-1.5 py-0.5 text-[9px] font-bold bg-green-900/30 text-green-400 border border-green-800/40 rounded">Set</button>
                    </div>
                  )}
                </div>
              </td>
          ...
          <tr key={order.id}>
            ...
            <td>
              ...
              {order.message && order.status === 'REJECTED' && (
                <div className="text-[9px] text-red-400/70 mt-0.5 max-w-[180px] truncate" title={order.message}>
                  {order.message}
                </div>
              )}
            </td>
...
function EmptyState({ message, icon }: { message: string; icon?: React.ReactNode }) {
  return (
    <div className="flex flex-col items-center justify-center h-full text-fw-text-muted py-6 gap-2">
      ...
      <span className="text-[9px] text-fw-text-muted/60">Data will appear here when market is active</span>
    </div>
  );
}
