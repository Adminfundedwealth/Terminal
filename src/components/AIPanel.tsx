    ...
    <div className="h-full flex flex-col bg-fw-surface overflow-hidden">
      ...
      <div className="flex items-center justify-between px-3 py-2 border-b border-fw-border bg-[#10121a] flex-shrink-0">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[12px] font-bold text-fw-text">AI Workspace</span>
        </div>
...
function TradeReviewTab() {
  ...
  return (
    <div className="p-3 space-y-3">
      <div className="text-[11px] text-fw-text-secondary mb-2">
      ...
      <div className="space-y-1 max-h-[200px] overflow-y-auto">
        {recentTrades.map(t => (
          ...
          >
            <div className="flex items-center gap-2">
              <span className={cn('text-[10px] font-bold px-1.5 py-0.5 rounded', t.side === 'BUY' ? 'bg-green-900/30 text-green-400' : 'bg-red-900/30 text-red-400')}>{t.side}</span>
              <span className="text-[11px] font-semibold text-fw-text">{t.symbol}</span>
              <span className="text-[10px] text-fw-text-muted">{t.date}</span>
            </div>
            <span className={cn('text-[11px] font-mono font-bold', t.pnl >= 0 ? 'text-green' : 'text-red')}>
      ...
      {selectedReview && (
        <div className="border border-fw-border rounded p-3 bg-fw-bg/50 space-y-3">
          <div className="text-[11px] font-bold text-fw-text">Analysis</div>
          ...
          <p className="text-[11px] text-fw-text-secondary leading-relaxed">{selectedReview.explanation}</p>
          <div className="px-2 py-1.5 bg-fw-purple/5 border border-fw-purple/20 rounded text-[10px] text-fw-purple">
  ...
  return (
    <div className="p-3 space-y-3">
      ...
      <div className="flex items-center justify-between p-3 bg-fw-bg border border-fw-border rounded">
        <div>
          <div className="text-[10px] text-fw-text-muted uppercase">Consistency Score</div>
          <div className="text-[9px] text-fw-text-secondary">{analysis.totalAnalyzed} trades over {analysis.tradingDays} days</div>
        </div>
...
function DailySummaryTab() {
  ...
  return (
    <div className="p-3 space-y-3">
      <div className="text-[11px] font-bold text-fw-text">Today's Summary</div>
      ...
      <div className="flex items-center gap-4 px-3 py-2 bg-fw-bg border border-fw-border rounded">
        <div className="flex items-center gap-1">
          <span className="text-[10px] text-fw-text-muted">Wins:</span>
          <span className="text-[12px] font-mono font-bold text-green">{summary.wins}</span>
        </div>
        <div className="flex items-center gap-1">
          <span className="text-[10px] text-fw-text-muted">Losses:</span>
          <span className="text-[12px] font-mono font-bold text-red">{summary.losses}</span>
        </div>
        <div className="flex items-center gap-1">
          <span className="text-[10px] text-fw-text-muted">Open Positions:</span>
          <span className="text-[12px] font-mono font-bold text-fw-text">{summary.openPositions}</span>
        </div>
      </div>
      ...
      {summary.suggestions.length > 0 && (
        <div className="space-y-1.5">
          <div className="text-[10px] text-fw-text-secondary font-bold uppercase">Insights & Suggestions</div>
          {summary.suggestions.map((s, i) => (
            <div key={i} className="flex items-start gap-2 px-3 py-2 bg-fw-purple/5 border border-fw-purple/15 rounded">
              ...
              <span className="text-[11px] text-fw-text-secondary leading-relaxed">{s}</span>
            </div>
          ))}
        </div>
      )}
...
function CoachingTab() {
  ...
  return (
    <div className="p-3 space-y-3">
      ...
      <div className="flex items-center gap-4 px-3 py-2 bg-fw-bg border border-fw-border rounded">
        <div>
          <div className="text-[9px] text-fw-text-muted uppercase">Last 7 Days</div>
          <div className="text-[12px] font-mono font-bold text-fw-text">{coaching.totalTrades} trades</div>
        </div>
        <div>
          <div className="text-[9px] text-fw-text-muted uppercase">Week P&L</div>
          <div className={cn('text-[12px] font-mono font-bold', coaching.weekPnl >= 0 ? 'text-green' : 'text-red')}>
            ₹{coaching.weekPnl.toLocaleString('en-IN')}
          </div>
        </div>
        <div>
          <div className="text-[9px] text-fw-text-muted uppercase">Win Rate</div>
          <div className="text-[12px] font-mono font-bold text-fw-text">{(coaching.winRate * 100).toFixed(0)}%</div>
        </div>
      </div>

      {/* Coaching Tips */}
      <div className="space-y-2">
        <div className="text-[10px] text-fw-text-secondary font-bold uppercase">Coaching Advice</div>
        {coaching.tips.map((tip, i) => (
          <div key={i} className="p-3 bg-fw-bg/50 border border-fw-border rounded">
            <div className="text-[10px] font-bold text-fw-purple mb-1">{tip.category}</div>
            <p className="text-[11px] text-fw-text-secondary leading-relaxed">{tip.advice}</p>
          </div>
        ))}
      </div>
...
function InsufficientDataMessage({ message, count }: { message: string; count: number }) {
  return (
    <div className="flex flex-col items-center justify-center h-full gap-3 px-6 text-center">
      ...
      <div className="text-[12px] text-fw-text-secondary">{message}</div>
      <div className="text-[11px] text-fw-text-muted">{count} trade{count !== 1 ? 's' : ''} available (need 5+)</div>
    </div>
  );
}
...
function ScoreCard({ label, score }: { label: string; score: number }) {
  ...
  return (
    <div className="bg-fw-bg border border-fw-border rounded p-2 text-center">
      <div className="text-[9px] text-fw-text-muted uppercase">{label}</div>
      <div className={cn('text-[18px] font-black font-mono', color)}>{score}</div>
      <div className="text-[9px] text-fw-text-muted">/10</div>
    </div>
  );
}
  ...
  return (
    <div className={cn('p-2 rounded border', detected ? 'border-red-800/30 bg-red-900/10' : 'border-fw-border bg-fw-bg/50')}>
      <div className="flex items-center gap-1.5 mb-0.5">
        ...
        <span className="text-[10px] font-bold text-fw-text">{label}</span>
      </div>
      <span className="text-[10px] text-fw-text-muted leading-relaxed">{detail}</span>
    </div>
  );
}
