  ...
  return (
    <div className="h-full flex flex-col bg-fw-surface">
      <div className="flex items-center justify-between px-3 py-2 border-b border-fw-border bg-fw-surface flex-shrink-0">
        <span className="text-base font-bold text-fw-text">Chart Templates</span>
        <span className="text-xs text-fw-text-secondary">{templates.length} saved</span>
      </div>
      ...
      <div className="px-3 py-2 border-b border-fw-border flex items-center gap-2 flex-shrink-0">
        <input
          ...
          className="flex-1 bg-fw-bg border border-fw-border rounded text-sm px-2 py-1.5 text-fw-text outline-none focus:border-fw-accent"
        />
        <button onClick={handleSave} className="flex items-center gap-1 px-2 py-1.5 text-xs bg-fw-accent text-white rounded font-bold hover:brightness-110">
          <Save size={10} /> Save Current
        </button>
      </div>

      {/* Current state */}
      <div className="px-3 py-1.5 border-b border-fw-border/30 flex items-center gap-3 text-xs text-fw-text-secondary flex-shrink-0">
          ...
          templates.map(tpl => (
            <div key={tpl.id} className="flex items-center justify-between px-3 py-2 border-b border-fw-border/30 hover:bg-fw-hover/30 group">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <div className="text-base font-semibold text-fw-text">{tpl.name}</div>
      ...
      {toast && <div className="px-3 py-1.5 border-t border-fw-border text-sm text-fw-accent bg-fw-accent/5">{toast}</div>}
    </div>
  );
}
