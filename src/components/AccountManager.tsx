    ...
    <div className="h-full flex flex-col bg-fw-surface overflow-hidden">
      ...
      <div className="flex items-center justify-between px-3 py-2 border-b border-fw-border bg-fw-surface flex-shrink-0">
        <div className="flex items-center gap-2">
          ...
          <span className="text-base font-bold text-fw-text">Account Manager</span>
        </div>
        <div className="flex items-center gap-1">
          <button
            onClick={() => setActiveView('config')}
            className={cn('text-xs px-2 py-1 rounded', activeView === 'config' ? 'bg-fw-accent text-white' : 'text-fw-text-secondary hover:bg-fw-hover')}
          >
            Config
          </button>
          <button
            onClick={() => { setActiveView('exposure'); refreshExposures(); }}
            className={cn('text-xs px-2 py-1 rounded', activeView === 'exposure' ? 'bg-fw-accent text-white' : 'text-fw-text-secondary hover:bg-fw-hover')}
          >
            Exposure
          </button>
        </div>
      ...
      <div className="px-3 py-1.5 border-t border-fw-border flex-shrink-0 bg-fw-cyan/5">
        <span className="text-xs text-fw-cyan">
  ...
  return (
    <div className="p-3 space-y-3">
      ...
      <div className="flex items-center justify-between p-3 bg-fw-bg border border-fw-border rounded">
        <div className="flex items-center gap-2">
          <Power size={14} className={config.isActive ? 'text-green' : 'text-fw-text-secondary'} />
          <div>
            <div className="text-sm font-bold text-fw-text">Copy Trading</div>
            <div className="text-xs text-fw-text-secondary">{config.isActive ? 'Active — orders will replicate' : 'Inactive — no replication'}</div>
          </div>
        </div>
        <button
      ...
      <div className="p-3 bg-fw-bg border border-fw-border rounded">
        <div className="text-xs text-fw-text-secondary uppercase font-bold mb-2">Master Account</div>
        {config.masterAccountId ? (
          <div className="flex items-center justify-between">
            <span className="text-base font-mono font-bold text-fw-text">{config.masterAccountId}</span>
            <button onClick={onClearMaster} className="text-xs text-red-400 hover:text-red">Remove</button>
          </div>
        ) : (
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 bg-fw-surface border border-fw-border rounded text-sm px-2 py-1.5 text-fw-text outline-none focus:border-fw-accent"
            />
            <button
              onClick={() => { if (masterInput.trim()) { onSetMaster(masterInput.trim()); setMasterInput(''); } }}
              className="px-3 py-1.5 text-xs bg-fw-accent text-white rounded font-bold"
            >
              Set
            </button>
          </div>
      ...
      {/* Copy Mode */}
      <div className="p-3 bg-fw-bg border border-fw-border rounded">
        <div className="text-xs text-fw-text-secondary uppercase font-bold mb-2">Copy Mode</div>
        <div className="flex gap-1">
          {(['mirror', 'proportional', 'fixed-lot'] as CopyMode[]).map(mode => (
            <button
              ...
              className={cn(
                'flex-1 py-1.5 text-xs rounded capitalize font-medium border transition-all',
        ...
        <div className="text-xs text-fw-text-secondary mt-1.5">
      ...
      <div className="p-3 bg-fw-bg border border-fw-border rounded">
        <div className="flex items-center justify-between mb-2">
          <div className="text-xs text-fw-text-secondary uppercase font-bold">Slave Accounts ({config.slaves.length})</div>
          <button onClick={() => setShowAddSlave(!showAddSlave)} className="flex items-center gap-1 text-xs text-fw-accent hover:text-fw-accent-hover">
            <Plus size={10} /> Add
          </button>
        </div>
        ...
        {showAddSlave && (
          <div className="mb-3 p-2 border border-fw-border rounded bg-fw-surface space-y-2">
            <div className="grid grid-cols-2 gap-2">
              <input value={newSlave.accountId} onChange={e => setNewSlave({ ...newSlave, accountId: e.target.value })} placeholder="Account ID" className="bg-fw-bg border border-fw-border rounded text-xs px-2 py-1.5 text-fw-text outline-none" />
              <input value={newSlave.name} onChange={e => setNewSlave({ ...newSlave, name: e.target.value })} placeholder="Name" className="bg-fw-bg border border-fw-border rounded text-xs px-2 py-1.5 text-fw-text outline-none" />
            </div>
            <div className="grid grid-cols-2 gap-2">
              <div>
                <label className="text-xs text-fw-text-secondary">Copy Ratio</label>
                <input type="number" step="0.1" min="0.1" value={newSlave.copyRatio} onChange={e => setNewSlave({ ...newSlave, copyRatio: parseFloat(e.target.value) || 1 })} className="w-full bg-fw-bg border border-fw-border rounded text-xs px-2 py-1.5 text-fw-text font-mono outline-none" />
              </div>
              <div>
                <label className="text-xs text-fw-text-secondary">Fixed Lots</label>
                <input type="number" min="1" value={newSlave.fixedLots} onChange={e => setNewSlave({ ...newSlave, fixedLots: parseInt(e.target.value) || 1 })} className="w-full bg-fw-bg border border-fw-border rounded text-xs px-2 py-1.5 text-fw-text font-mono outline-none" />
              </div>
            </div>
            <button onClick={onAddSlave} className="w-full py-1.5 text-xs bg-fw-accent text-white rounded font-bold">Add Slave Account</button>
          </div>
        )}

        {/* Slave List */}
        {config.slaves.length === 0 ? (
          <div className="text-sm text-fw-text-secondary text-center py-4">No slave accounts configured</div>
        ) : (
            ...
            {config.slaves.map(slave => (
              <div key={slave.accountId} className="flex items-center justify-between p-2 border border-fw-border/50 rounded hover:bg-fw-hover/30">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <div className="text-sm font-semibold text-fw-text">{slave.name}</div>
                    <div className="text-xs text-fw-text-secondary font-mono">{slave.accountId}</div>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  <span className="text-xs text-fw-text-secondary font-mono">
...
function ExposureView({ exposures, loading, onRefresh }: { exposures: AccountExposure[]; loading: boolean; onRefresh: () => void }) {
  return (
    <div className="p-3 space-y-3">
      <div className="flex items-center justify-between">
        <div className="text-sm font-bold text-fw-text">Portfolio Exposure</div>
      ...
      {loading && (
        <div className="flex items-center justify-center h-20 text-fw-text-secondary text-sm">Loading exposures...</div>
      )}

      {!loading && exposures.length === 0 && (
        <div className="flex flex-col items-center justify-center h-32 gap-2 text-center">
          <AlertCircle size={24} className="text-fw-text-secondary/40" />
          <div className="text-sm text-fw-text-secondary">No exposure data available.</div>
          ...
          {exposures.map(exp => (
            <div key={exp.accountId} className="p-2 bg-fw-bg border border-fw-border rounded">
              <div className="flex items-center justify-between mb-1">
                <span className="text-sm font-bold text-fw-text">{exp.accountName}</span>
                <span className={cn('text-sm font-mono', exp.totalMtm >= 0 ? 'text-green' : 'text-red')}>
                  ₹{exp.totalMtm.toLocaleString('en-IN')}
                </span>
              </div>
              <div className="flex items-center gap-3 text-xs text-fw-text-secondary">
