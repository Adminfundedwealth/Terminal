...
export function OptionChainModal() {
  ...
  const scheduleRetry = useCallback((
    ...
    if (attempt >= MAX_AUTO_RETRIES) {
      // Check if the error indicates a market-closed / service-unavailable scenario
  ...
  const startSymbolLoad = useCallback(async (sym: string, session: number) => {
    ...
    try {
      ...
      if (!isMountedRef.current) return;
      if (session !== sessionRef.current) return; // symbol changed during expiry fetch
    ...
    // Use fallback if backend returned nothing
    ...
    // Load chain directly — do NOT rely on a useEffect watching selectedExpiry.
  ...
  useEffect(() => {
    // Increment session — any in-flight request for the old symbol is now stale
  ...
  // ^ symbol is the only dep; startSymbolLoad/clearPendingTimers are stable useCallbacks
  ...
  const handleSymbolChange = useCallback((sym: string) => {
    ...
    setSymbol(sym); // triggers useEffect([symbol]) above
  }, [symbol]);

  // ── User-initiated: manual retry ─────────────────────────────────────────
  // Reads symbol and selectedExpiry from current state — always current values.
