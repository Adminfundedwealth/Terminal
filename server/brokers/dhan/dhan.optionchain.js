  ...
  _parseOCMap(data) {
    ...
    for (const [strikeStr, sides] of Object.entries(oc)) {
      ...
      chain.push({
        ...
        // Bid/Ask — Dhan v2 uses top_bid_price/top_ask_price + quantities.
        // Older shapes (bid/ask/bid_price) kept as fallbacks; never fabricated.
        callBidPrice: parseFloat(ce.top_bid_price || ce.bid || ce.bid_price || 0),
        callAskPrice: parseFloat(ce.top_ask_price || ce.ask || ce.ask_price || 0),
        callBidQty:   parseInt(ce.top_bid_quantity || ce.bid_qty || ce.bid_quantity || 0),
        callAskQty:   parseInt(ce.top_ask_quantity || ce.ask_qty || ce.ask_quantity || 0),
        // Previous close — used to derive change/change% downstream (0 = unavailable)
        callPrevClose: parseFloat(ce.previous_close_price || ce.prev_close || ce.close || 0),
        ...
        putBidPrice: parseFloat(pe.top_bid_price || pe.bid || pe.bid_price || 0),
        putAskPrice: parseFloat(pe.top_ask_price || pe.ask || pe.ask_price || 0),
        putBidQty:   parseInt(pe.top_bid_quantity || pe.bid_qty || pe.bid_quantity || 0),
        putAskQty:   parseInt(pe.top_ask_quantity || pe.ask_qty || pe.ask_quantity || 0),
        putPrevClose: parseFloat(pe.previous_close_price || pe.prev_close || pe.close || 0),
  ...
  _parseChain(data) {
    ...
    for (const item of data) {
      ...
      chain.push({
        ...
        callBidPrice: parseFloat(item.ce_top_bid_price || item.ce_bid || item.CE_BidPrice || 0),
        callAskPrice: parseFloat(item.ce_top_ask_price || item.ce_ask || item.CE_AskPrice || 0),
        callBidQty: parseInt(item.ce_top_bid_quantity || item.ce_bid_qty || item.CE_BidQty || 0),
        callAskQty: parseInt(item.ce_top_ask_quantity || item.ce_ask_qty || item.CE_AskQty || 0),
        callPrevClose: parseFloat(item.ce_previous_close_price || item.ce_prev_close || item.CE_PrevClose || 0),
        ...
        putBidPrice: parseFloat(item.pe_top_bid_price || item.pe_bid || item.PE_BidPrice || 0),
        putAskPrice: parseFloat(item.pe_top_ask_price || item.pe_ask || item.PE_AskPrice || 0),
        putBidQty: parseInt(item.pe_top_bid_quantity || item.pe_bid_qty || item.PE_BidQty || 0),
        putAskQty: parseInt(item.pe_top_ask_quantity || item.pe_ask_qty || item.PE_AskQty || 0),
        putPrevClose: parseFloat(item.pe_previous_close_price || item.pe_prev_close || item.PE_PrevClose || 0),
